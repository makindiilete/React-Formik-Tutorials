/*
We will learn how to manually trigger both the form and field level validations using formik...
To manually trigger form validation, we need to use the render props pattern on the entire form itself..
*/
      ...
      {(formik) => {
        ...
        return (
          <Form>
            ...
            {/*to validate a given field, we call d validateField() method passing d field name*/}
            <button
              type="button"
            ...
            {/*to validate the entire form, we call validateForm() with no argument*/}
            <button
              type="button"
              className="btn btn-outline-primary btn-block"
...
/*
#   Now if we test this out, and click on the Validate Comment, the error object we get in the console now contain the validation message for comment field : -

Formik prop =
{values: {…}, errors: {…}, touched: {…}, status: undefined, isSubmitting: false, …}
dirty: false
errors:
comment: "Comments Cannot Be Empty"
__proto__: Object


#   If we do the same for the second button to manually trigger validation for all fields, we get the error object filled with all the validations
...
But the issue is, even though we are getting the error object populated with the errors, we are not seeing any changes in the UI, this validation errors are not showing up in the UI.. Why?

This is because for the errors to show, the field must be touched and with the manually validation, the fields are not touched.. So we also manually need to set the fields as touched before triggering the manuall validation..
*/
